    ...
    void OnGUI()
{
    ...
    EditorGUILayout.HelpBox("กรุณาตรวจสอบว่ามี Tag ดังต่อไปนี้: \"Bullet\", \"Enemy\"", MessageType.Info);
    ...
    public static void MakeSelectedObjectPrefab()
    {
        ...
        if (selected == null)
        {
            EditorUtility.DisplayDialog("Invalid GameObject", "Please Select GameObject on Hierarchy first", "OK");
            return;
        }
        ...
        if (prefab != null)
        {
            ...
            EditorUtility.DisplayDialog("Success", $"Create Prefab at: {prefabPath}", "OK");
        }
        else
        {
            EditorUtility.DisplayDialog("Failed", "Can't create Prefab!", "OK");
        }
    }
}
